...
Phase 3 categories shipped through task 14: **`scene.*`** (15), **`project.*`** (7), **`node.*`** (14),
**`script.*`** (8), **`signal.*`** (10), **`resource.*`** (15), **`shader.*`** (6), plus 3 legacy server methods.
...
- Replaced invalid `ResourceLoader.get_resource_type()` (not in Godot 4.6) with `resource_class_from_path()` (load + `.tres` header sniff).
- Headless `shader.compile_check` uses probe-uniform compile heuristic (Godot does not expose structured compile errors at runtime).
...
- Editor-only resource methods (`rename`, `replace_references`, `set_uid`) return `editor.not_available` in headless — documented in `docs/catalog/parity.md`.
- `shader.compile_check` probe-uniform heuristic may miss edge cases where drivers optimize out unused uniforms.
