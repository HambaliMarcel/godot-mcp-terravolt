...
Formal intent: **dual-stack** MCP for Godot 4.x (`.NET-compatible` target), exceeding aggregate patterns from **`youichi-uda/godot-mcp-pro`**, **`tomyud1/godot-mcp`**, and **`Coding-Solo/godot-mcp`**.
...
| [**00-fundamentals-contract.md**](00-fundamentals-contract.md) | **Baseline contracts** locked *before* Phase 1 coding (constants, gates, layering) |
...
3. Tool registry — **what** the long-term surface looks like; wire tools only when the roadmap phase allows.

Implement shippable code under **`packages/mcp-server/`** and **`packages/godot-mcp-addon/`** ([packages/README.md](../../packages/README.md)).
