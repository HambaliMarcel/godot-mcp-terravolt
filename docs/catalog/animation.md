...
**v1 notes:** `animation.list` uses the edited scene in the editor; headless loads
`application/run/main_scene` or instantiates scenes for `scope: "project"`. `animation.play` and
`animation.preview_export` target runtime/editor respectively; preview export degrades to a PNG
sequence when FFmpeg is unavailable.
