...
**v1 notes:** State-machine CRUD requires an `AnimationTree` whose `tree_root` is an `AnimationNodeStateMachine`. `set_parameter` with `mode: "travel"` expects a `…/playback` parameter of type `AnimationNodeStateMachinePlayback`.
